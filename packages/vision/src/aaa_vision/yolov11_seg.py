...
from aaa_core.config.console import info, status, underline  # noqa: E402
...
class YOLOv11Seg:
    def __init__(self, model_size="n"):
        ...
        # Load YOLOv11 segmentation model from data/models/ directory
        ...
        with suppress_stderr():
            if os.path.exists(local_model):
                ...
                # Move downloaded model to data/models/ directory for future use
                if (os.path.exists(model_name) and
                        not os.path.exists(local_model)):
                    import shutil
                    shutil.move(model_name, local_model)
                    status(f"Moved {model_name} to data/models/ directory")
