...
def view_with_open3d(points: np.ndarray, path: str = None):
    ...
    znorm = (zvals - zvals.min()) / (zvals.ptp() + 1e-9)
    ...
    vis.register_key_callback(ord('A'), lambda vis_obj=None: _rotate(vis_obj, yaw_deg=-10.0))
    vis.register_key_callback(ord('D'), lambda vis_obj=None: _rotate(vis_obj, yaw_deg=10.0))
    # Map U/J for up/down rotation to avoid common conflicts (e.g., S/X used elsewhere)
    vis.register_key_callback(ord('U'), lambda vis_obj=None: _rotate(vis_obj, pitch_deg=10.0))
    vis.register_key_callback(ord('u'), lambda vis_obj=None: _rotate(vis_obj, pitch_deg=10.0))
    vis.register_key_callback(ord('J'), lambda vis_obj=None: _rotate(vis_obj, pitch_deg=-10.0))
    vis.register_key_callback(ord('j'), lambda vis_obj=None: _rotate(vis_obj, pitch_deg=-10.0))
    ...
    try:
        ...
        vis.register_key_callback(262, lambda vis_obj=None: _rotate(vis_obj, yaw_deg=10.0))
        vis.register_key_callback(263, lambda vis_obj=None: _rotate(vis_obj, yaw_deg=-10.0))
        vis.register_key_callback(265, lambda vis_obj=None: _rotate(vis_obj, pitch_deg=10.0))
        vis.register_key_callback(264, lambda vis_obj=None: _rotate(vis_obj, pitch_deg=-10.0))
    except Exception:
        pass
    ...
    print("Keyboard shortcuts: R=reset, X/Y/Z=align axis, A/D=rotate left/right, U/J=rotate up/down, +/-=zoom, F=fit, H=help")

    vis.run()
    vis.destroy_window()


def view_with_matplotlib(points: np.ndarray, max_points: int = 200000, path: str = None):
    ...
    help_box = fig.text(0.02, 0.98, help_text, fontsize=9, va='top', ha='left', bbox=dict(facecolor='white', alpha=0.75))
    ...
    def on_key(event):
        ...
        if key.lower() == 'r':
            ...
            def zoom(factor):
                for axis_get, axis_set in ((ax.get_xlim3d, ax.set_xlim3d), (ax.get_ylim3d, ax.set_ylim3d), (ax.get_zlim3d, ax.set_zlim3d)):
                    mn, mx = axis_get()
                    c = 0.5 * (mn + mx)
            ...
            fig.canvas.draw_idle()
            print(f"[Cloudview] Help overlay {'ON' if help_box.get_visible() else 'OFF'} (matplotlib)")

    fig.canvas.mpl_connect('key_press_event', on_key)

    print("Keyboard shortcuts (matplotlib): R=reset, X/Y/Z=align, A/D/U/J or arrow keys=rotate, +/-=zoom, F=fit, H=help")
    plt.show()
...
def main():
    ...
    parser.add_argument('--no-open3d', action='store_true', help='Force matplotlib viewer')
    parser.add_argument('--subsample', type=int, default=4, help='Subsample factor for dense clouds (matplotlib viewer)')
    ...
    try:
        ...
        if not args.no_open3d:
            try:
                ...
                print(f"Open3D viewer unavailable or failed: {e}\nFalling back to matplotlib")

        view_with_matplotlib(points, max_points=200000 // max(1, args.subsample), path=path)
